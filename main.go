...
//go:embed assets/favicon.svg assets/favicon.ico
...
func serveFaviconSVG(w http.ResponseWriter, r *http.Request) {
    content, err := faviconFS.ReadFile("assets/favicon.svg")
    if err != nil {
        http.Error(w, "Internal Server Error", http.StatusInternalServerError)
        return
    }
...
func serveFaviconICO(w http.ResponseWriter, r *http.Request) {
    content, err := faviconFS.ReadFile("assets/favicon.ico")
    if err != nil {
        http.Error(w, "Internal Server Error", http.StatusInternalServerError)
        return
    }
...
func main() {
    ...
    if *https {
        if _, err := os.Stat(*certFile); os.IsNotExist(err) {
            err := generateSelfSigned(*certFile, *keyFile)
            if err != nil {
